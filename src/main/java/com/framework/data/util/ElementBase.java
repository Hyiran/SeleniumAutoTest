...
public class ElementBase {
    ...
    public boolean isFocus() {
        ...
        return Boolean.valueOf(element.getAttribute("focused"));
    }
    ...
    public void swipe(String direction) {
        ...
        if (!direction.isEmpty()) {
            if (direction.equalsIgnoreCase("left")) {
                ((AppiumDriver) driver).swipe(width - 20, height / 2, 20, height / 2, 600);
            } else if (direction.equalsIgnoreCase("right")) {
                log.info("右滑[" + "]");
                ((AppiumDriver) driver).swipe(20, height / 2, width - 20, height / 2, 200);
            } else if (direction.equalsIgnoreCase("up")) {
                ((AppiumDriver) driver).swipe(width / 2, 50, width / 2, height - 50, 200);
            } else if (direction.equalsIgnoreCase("down")) {
                ((AppiumDriver) driver).swipe(width / 2, height - 50, width / 2, 50, 200);
            }
        }
    ...
    public void longPress() {
        // TODO Auto-generated method stub
        action.longPress(element.getWebElement());
    }
...
//    scrollIntoView();
    ...
    public void valueToBeNotEmpty() {
        new WebDriverWait(this.driver, Long.parseLong(System.getProperty("WebDriver.webDriverWait"))).until(new Function<WebDriver, Boolean>() {
            public Boolean apply(WebDriver driver) {
                if (ElementBase.this.getAttribute("value").length() != 0) return true;
                return false;
            }
        });
    }
